...
(function (root, factory) {
  ...
  if (typeof define === 'function' && define.amd) {
    // AMD + global
    define([], function () {
      return root.ritaAnnyang = factory(root);
    });
  ...
  var initIfNeeded = function initIfNeeded() {
    if (!isInitialized()) {
      ritaAnnyang.init({}, false);
    }
  };

  var registerCommand = function registerCommand(command, callback, originalPhrase, scope = 'Unknown') {
    commandsList.push({ command: command, callback: callback, originalPhrase: originalPhrase , scope: scope});
    if (debugState) {
  ...
  ritaAnnyang = {
     ...
     */
    addCommands: function addCommands(commands, scope = 'Unknown') {
      var cb;
    ...
    getCommandsList: function getCommandsList() {
      return commandsList;
    },
    ...
    trigger: function trigger(sentences) {
      if (!ritaAnnyang.isListening()) {
        if (debugState) {
          if (!_isListening) {
            logMessage('Cannot trigger while ritaAnnyang is aborted');
          } else {
            logMessage('Speech heard, but ritaAnnyang is paused');
          }
        }
        return;
      }
      ...
      recognition.onend = function () {
        ...
        if (autoRestart) {
          ...
          if (timeSinceLastStart < 1000) {
            setTimeout(function () {
              ritaAnnyang.start({ paused: pauseListening });
